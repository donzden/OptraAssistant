...
import { disconnectZerodha, importFromZerodha } from '@/api/portfolio'
...
export default function ZerodhaConnectBanner({ connected, onStatusChange, onImported }: Props) {
  const handleConnect = () => {
    window.location.href = '/api/v1/kite/auth'
  }
  ...
  const handleImport = async () => {
    const t = toast.loading('Importing positions from Zerodha…')
    try {
      const result = await importFromZerodha()
  ...
  if (connected) {
    return (
      <div className="flex items-center gap-3 bg-emerald-900/20 border border-emerald-700/40 rounded-xl px-4 py-3 text-sm">
        ...
        <span className="text-emerald-300 font-medium">Zerodha connected</span>
        <span className="text-slate-500 text-xs ml-1">(session resets at 6 AM IST)</span>
        <button
  ...
  return (
    <div className="flex items-center gap-3 bg-indigo-900/20 border border-indigo-700/40 rounded-xl px-4 py-3 text-sm">
      ...
      <div>
        <span className="text-slate-300">Connect your Zerodha account to auto-import live positions. </span>
        <span className="text-slate-400 text-xs">Or add positions manually below.</span>
      </div>
      <button
        onClick={handleConnect}
        className="ml-auto shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#387ED1] hover:bg-[#2e6ab8] text-white text-xs font-medium transition-colors"
      >
        <Link2 size={13} />
        Connect Zerodha
      </button>
